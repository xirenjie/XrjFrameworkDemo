...
import com.xrj.framework.livelayout.CustomRoundView;
import com.xrj.framework.livelayout.DisplayUtil;
import com.xrj.framework.livelayout.HorizontalListView;
import com.xrj.framework.livelayout.MagicTextView;
import com.xrj.framework.livelayout.SoftKeyBoardListener;
import com.xrj.imframework.bean.GroupResponseBean;
...
public class LayerFragment extends Fragment implements View.OnClickListener {
    ...
    private CustomRoundView custom_imgs;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        custom_imgs= (CustomRoundView) view.findViewById(R.id.custom_imgs);
        custom_imgs.setOnClickListener(this);
        ...
        view.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (llInputParent.getVisibility() == View.VISIBLE) {
                    tvChat.setVisibility(View.VISIBLE);//显示聊天按钮
                    llInputParent.setVisibility(View.GONE);//消息发送窗口隐藏
                    hideKeyboard();//隐藏软键盘
                }
            }
        });
        softKeyboardListnenr();//软键盘监听
        setMessageData();//消息列表
        startTimer();//计时器
        filter();
    }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (aciton.equals("com.xrjframework.demo.websocket.data")) {
               ...
               try
               {
                   ...
                   if(content==11)
                   {
                       GroupResponseBean liveBean=new Gson().fromJson(mass,GroupResponseBean.class);
                       messageData.add(liveBean.getData().getFrom() + ": " + liveBean.getData().getContent());
                       etInput.setText("");
            ...
            {
                Toast.makeText(getActivity(),"当前连接已断开",Toast.LENGTH_SHORT).show();
                getActivity().finish();
            }
    ...
    public void setMessageData()
    {
        ...
        messageData.add("系统通知: 欢迎来到直播间");
        messageAdapter=new MessageAdapter(getActivity(), messageData);
        lvmessage.setAdapter(messageAdapter);
        lvmessage.setSelection(messageData.size());//定位到当前条目
        hlvaudience.setAdapter(new AudienceAdapter(getActivity()));//加载观众列表
    }
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.tvSendone:/*礼物1*/
                WsManager.getInstance().sendUserMessage();
                showGift("X1");
                break;
            ...
            case R.id.tvSendfor:/*礼物4*/
                showGift("X4");
                break;
            case R.id.custom_imgs:/*指定某人退出群组*/
                Toast.makeText(getActivity(),"指定某人退出群组",Toast.LENGTH_SHORT).show();
                WsManager.getInstance().SpecifyExit("1538967570911");
                break;
            case R.id.tvqq://针对某个人禁言
                WsManager.getInstance().sendChatText("禁言","xirenjie","yangxuan", getTime(),2+"","","","张三");
                break;
        }
    }
    ...
    private void showGift(String tag) {
        ...
        if (giftView == null) {/*该用户不在礼物显示列表*/
            ...
            final CustomRoundView crvgift= (CustomRoundView) giftView.findViewById(R.id.ivgift);//礼物控件
    ...
    private void showChat() {
        ...
        llInputParent.requestFocus();//获取焦点
        showKeyboard();//显示软键盘 隐藏头布局
    }
    ...
    private void sendText() {
        ...
        if(!etInput.getText().toString().trim().isEmpty()){
            WsManager.getInstance().sendGrouptext(etInput.getText().toString().trim(), "yangxuan", getTime(),1+"","","","李四");
            etInput.setText("");
            hideKeyboard();
        }else
            hideKeyboard();
